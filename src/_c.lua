-- Client-side script for Qbox/MRI Qbox

local QBCore = exports['qb-core']:GetCoreObject() -- Access Qbox core
local wall = false
local walldistance = 500 -- OneSync Infinity Max Distance
local admin = true -- Assume admin for now; should be checked server-side
local armas = {
    ...
    [tostring(GetHashKey('WEAPON_APPISTOL'))] = 'AP Pistol', [tostring(GetHashKey('WEAPON_ASSAULTRIFLE'))] = 'Assault Rifle', [tostring(GetHashKey('WEAPON_ASSAULTRIFLE_MK2'))] = ' unimaginative Mk2',
...
-- Toggle wallhack event
RegisterNetEvent('qbox_wall:toggle')
AddEventHandler('qbox_wall:toggle', function(val)
    wall = val
...
-- Thread to periodically fetch wallhack user data
Citizen.CreateThread(function()
    -- Trigger initial server-side setup if needed
    TriggerServerEvent('qbox_wall:setWallInfos')
    while true do
        local time = 5000
        if wall then
            -- Fetch wallhack user data via server callback
            QBCore.Functions.TriggerCallback('qbox_wall:getWallInfos', function(users)
                wall_users = users
            end)
            time = 2000
        end
...
-- Thread to render wallhack information
Citizen.CreateThread(function()
    while true do
        local time = 2000
        if wall then
            ...
            for k, id in ipairs(GetActivePlayers()) do
                ...
                if NetworkIsPlayerActive(id) then
                    local x1, y1, z1 = table.unpack(GetEntityCoords(ped_id, true))
                    local x2, y2, z2 = table.unpack(GetEntityCoords(nped_id, true))
                    local distance = math.floor(GetDistanceBetweenCoords(x1, y1, z1, x2, y2, z2, true))
                    local x, y, z = table.unpack(GetPedBoneCoords(nped_id, 0, 0.0, 0.0, -0.9))
                    local px, py, pz = table.unpack(GetGameplayCamCoord())
                    if nped_id ~= -1 and wall_users[src] then
                        if GetDistanceBetweenCoords(x, y, z, px, py, pz, true) <= walldistance then
                            ...
                            if armahash == 0 then
                                ...
                                arma = armas[tostring(armahash)] or nil
                            end

                            DrawTextW(x2, y2, z2 + 0.15, "~w~[" .. (wall_users[src].user_id or src) .. "] ~w~" .. (wall_users[src].name or "Unknown") .. " | src: " .. src)

                            local tab = "\n"
                            local defaulttext = tab .. "~w~Vida:~g~" .. health .. " ~w~|| Colete:~b~" .. armour .. "~w~"

                            if wall_users[src].wallstats then
